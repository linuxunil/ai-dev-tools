...
class BatchResultsAnalyzer:
    ...
    def analyze_performance_trends(self) -> Dict[str, Any]:
        ...
        for result in self.results:
            ...
            successful_configs = [
                config for config in result.get("configurations", [])
    ...
    def generate_summary_report(self, trends: Dict[str, Any]) -> str:
        ...
        if trends["by_batch_type"]:
            ...
            for batch_type, data in trends["by_batch_type"].items():
                ...
                report.append(f"- **{batch_type}**: {token_avg:.1f}% tokens, {time_avg:.1f}% time ({len(data)} samples)")
            report.append("")
        ...
        if trends["by_profile"]:
            ...
            report.append(f"- **Best performing profile**: {best_profile} ({profile_performance[best_profile]:.1f}% token reduction)")
        ...
        if trends["by_model"]:
            ...
            report.append(f"- **Best performing model**: {best_model} ({model_performance[best_model]:.1f}% token reduction)")
    ...
    def create_visualizations(self, trends: Dict[str, Any], output_dir: str = "visualizations"):
        ...
        try:
            ...
            if trends["by_profile"]:
                ...
                token_avgs = [statistics.mean([d["token_reduction"] for d in trends["by_profile"][p]]) 
                             for p in profiles]
                time_avgs = [statistics.mean([d["time_reduction"] for d in trends["by_profile"][p]]) 
                            for p in profiles]
...
def main():
    ...
    parser.add_argument("--create-visualizations",
                       action="store_true", 

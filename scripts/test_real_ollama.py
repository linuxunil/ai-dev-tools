...
"""
    ...
    with measure_workflow(WorkflowType.SAFETY_CHECK) as baseline_context:
        ...
        if baseline_result["success"]:
            baseline_context.record_tokens(
    ...
    structured_analysis = {
        ...
        "issues_found": [
            {"line": 7, "type": "code_injection", "pattern": "eval()"}
        ],
...
"""
    ...
    with measure_workflow(WorkflowType.SAFETY_CHECK) as tools_context:
        ...
        if tools_result["success"]:
            tools_context.record_tokens(
                tools_result["input_tokens"],
                tools_result["output_tokens"] 
            )
...
def main():
    ...
    try:
        ...
        if test_response.status_code == 200:
            ...
            print("💡 Make sure to pull the model: curl -X POST http://localhost:11434/api/pull -d '{\"name\": \"llama3.2:1b\"}'")
            return
    except Exception as e:
    ...
    if results:
        ...
        print(f"   • Structured, machine-readable output")
        print(f"   • Consistent, repeatable results")

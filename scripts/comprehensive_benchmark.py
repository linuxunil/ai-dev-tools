...
class BenchmarkTask:
    def __init__(
        ...
        expected_improvement: float = 30.0,
    ):
        self.name = name
...
"""
    ...
    return [
        ...
        BenchmarkTask(
            ...
            context_baseline,
            context_tools,
            WorkflowType.CONTEXT_ANALYSIS,
            25.0,
        ),
    ]


def run_benchmark_samples(task: BenchmarkTask, sample_size: int = 10, model: str = "llama3.2:1b") -> Dict[str, Any]:
    ...
    print("Running baseline samples...")
...
def calculate_statistics(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    stats = {
        "sample_sizes": {"baseline": len(baseline_tokens), "tools": len(tools_tokens)},
        "tokens": {
            "baseline": {
    ...
    token_improvement = (
        (stats["tokens"]["baseline"]["mean"] - stats["tokens"]["tools"]["mean"]) / stats["tokens"]["baseline"]["mean"]
    ) * 100

    time_improvement = (
        (stats["time"]["baseline"]["mean"] - stats["time"]["tools"]["mean"]) / stats["time"]["baseline"]["mean"]
    ) * 100
    ...
    for task_result in all_results["task_results"]:
        ...
        if "improvements" in stats:
            ...
            if improvements["statistical_significance"]:
                print("  ✅ Statistically significant (n≥10)")
            else:
                print("  ⚠️  Small sample size")

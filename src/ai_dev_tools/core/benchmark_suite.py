...
from .ollama_client import ModelSize
...
class BenchmarkSuite:
    ...
    def run_task_baseline(self, task: StandardizedTask, model: ModelSize) -> Dict[str, Any]:
        ...
        with measure_workflow(task.baseline_workflow) as _context:
    ...
    def run_task_with_tools(
        ...
        # Note: For now we just simulate, actual ollama client would be
        ...
        with measure_workflow(task.baseline_workflow) as _context:
            ...
            _context.record_tokens(simulated_tokens_in, simulated_tokens_out)
    ...
    def run_comparison_suite(
        self, models: List[ModelSize], ollama_hosts: List[str] = ["localhost:11434"]
    ) -> Dict[str, Any]:
...
def create_benchmark_report(results: Dict[str, Any], output_file: Optional[str] = None) -> str:
    ...
    for comp_id, comp in results["comparisons"].items():
        ...
        report.append(
            f"  Baseline: {baseline_metrics.get('total_tokens', 0):.0f} tokens, "
            f"{baseline_metrics.get('execution_time', 0):.1f}s"
        )
        report.append(
            f"  Tools:    {tools_metrics.get('total_tokens', 0):.0f} tokens, "
            f"{tools_metrics.get('execution_time', 0):.1f}s"
        )

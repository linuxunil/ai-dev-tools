        ...
        for ext in self.file_extensions:
            for file_path in search_path.rglob(f"*{ext}"):
                if file_path.is_file():
                    matches.extend(
                        self._scan_file_for_patterns(file_path, target_pattern)
                    )
    ...
    def _scan_file_for_patterns(
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                threshold = (
                    0.7
                    if target_pattern["pattern_type"]
                    in [PatternType.MKIF_LIST_CONCAT, PatternType.HOMEBREW_LIST]
                    else 0.3
        ...
        if format_output != "silent":
            if format_output == "compact":
                result = {
                    "c": len(matches),
                    "items": [
                        {"f": m.file, "l": m.line, "conf": m.confidence}
                        for m in matches[:10]
                    ],
                }
